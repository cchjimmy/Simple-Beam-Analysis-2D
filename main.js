...
function main() {
  ...
  // evalBeam(new Beam({'simply-supported': true, cantilever: false}, 10, 10, [new Force(0, -100000, 5)]));
  // evalBeam(new Beam({'simply-supported': true}, 12, 1, [new Force(0, -36000, 10)], [new DistForce(()=>-8000, 5, 0, 6)]));
  evalBeam(new Beam({'simply-supported':true}, 10, 1, [new Force(0, -10000, 8)], [new DistForce(()=>-20000, 2, 0, 4)]))
}

function Beam(type = {'simply-supported': true, 'cantilever': false}, length, height, forces = [], distributedForces = [], moments = []) {
  this.type = type;
  this.length = length;
  this.height = height;
  this.forces = {
...
function DistForce(func, posX = 0, posY = 0, length) {
